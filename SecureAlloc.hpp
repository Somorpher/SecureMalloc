template <typename _cT, typename = std::enable_if<!std::is_reference_v<_cT> && !std::is_pointer_v<_cT>>> class SecureAlloc
{
  ...
  public:
    __attribute__((nothrow, leaf, optimize("3"), section(".text"))) explicit SecureAlloc() noexcept
    {
    ...
    __attribute__((nothrow, leaf, optimize("3"), access(read_only, 1))) explicit SecureAlloc(const _cT _Pv) noexcept
    {
    ...
    __attribute__((nothrow, leaf, optimize("3"), access(read_only, 1))) explicit SecureAlloc(const SecureAlloc &_o) noexcept : _block_size_(_o._block_size_), _Mtx_(_o._Mtx_), _is_free_(_o._is_free_), _is_init_(_o._is_init_)
    {
        *this->_data_ = std::move(*_o._data_);
    };
    __attribute__((nothrow, leaf, optimize("3"), access(read_only, 1))) SecureAlloc &operator=(const SecureAlloc &_o) noexcept
    {
        if (*this != _o)
        {
            this->_data_ = std::move(*(_o._data_));
    ...
    __attribute__((nothrow, leaf, optimize("3"), access(read_write, 1))) explicit SecureAlloc(SecureAlloc &&_o) noexcept : _data_(std::move_if_noexcept(_o._data_)), _block_size_(_o._block_size_), _Mtx_(std::move(_o._Mtx_)), _is_free_(_o._is_free_), _is_init_(_o._is_init_)
    {
        _o.~SecureAlloc();
    };
    __attribute__((nothrow, leaf, optimize("3"), access(read_write, 1))) SecureAlloc &operator=(SecureAlloc &&_o) noexcept
    {
        if (this != _o)
        {
    ...
    constexpr inline bool operator==(const SecureAlloc &_o) noexcept
    {
